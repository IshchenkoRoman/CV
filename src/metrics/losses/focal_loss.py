...
class FocalLoss(ILoss):
    ...
    def focal_loss(self, y_real, y_pred, eps = 1e-8, gamma = 2):
        ...
        y_pred =  torch.clamp(torch.sigmoid(y_pred), min=eps, max=1e8)# hint: torch.clamp
        loss = - torch.sum(torch.pow(1-y_pred, gamma)*y_real*torch.log(y_pred)+(1-y_real)*torch.log(torch.clamp(1-y_pred, min=eps, max=1e8)))
